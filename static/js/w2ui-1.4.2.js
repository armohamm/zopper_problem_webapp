...
(function () {
    var w2grid = function(options) {
        ...
        this.records      = [];       // { officename: int(requied), field1: 'value1', ... fieldN: 'valueN', style: 'string', editable: true/false, summary: true/false, changes: object }
    ...
    $.fn.w2grid = function(method) {
        if (typeof method === 'object' || !method ) {
            ...
            // check if there are records without officename
            for (var r in records) {
                if (records[r].officename == null || typeof records[r].officename == 'undefined') {
                    console.log('ERROR: Cannot add records without officename. (obj: '+ object.officename +')');
                    return;
                }
    ...
    w2grid.prototype = {
        ...
        add: function (record) {
            ...
            var added = 0;
            for (var o in record) {
                if (!this.officename && typeof record[o].officename == 'undefined') record[o].officename = record[o][this.officename];
                if (record[o].officename == null || typeof record[o].officename == 'undefined') {
                    console.log('ERROR: Cannot add record without officename. (obj: '+ this.name +')');
                    continue;
                }
        ...
        find: function (obj, returnIndex) {
            ...
            for (var i = 0; i < this.records.length; i++) {
                ...
                if (match && returnIndex !== true) recs.push(this.records[i].officename);
                if (match && returnIndex === true) recs.push(i);
            }
            return recs;
        },

        set: function (officename, record, noRefresh) { // does not delete existing, but overrides on top of it
            if (typeof officename == 'object') {
                noRefresh = record;
                record    = officename;
                officename     = null;
            }
            ...
            if (officename == null) {
                for (var r in this.records) {
                    $.extend(true, this.records[r], record); // officename is the whole record
                }
                if (noRefresh !== true) this.refresh();
            } else { // find record to update
                var ind = this.get(officename, true);
                if (ind == null) return false;
                var isSummary = (this.records[ind] && this.records[ind].officename == officename ? false : true);
                if (isSummary) {
                    $.extend(true, this.summary[ind], record);
                } else {
                    $.extend(true, this.records[ind], record);
                }
                if (noRefresh !== true) this.refreshRow(officename); // refresh only that record
            }
            return true;
        },

        get: function (officename, returnIndex) {
            // search records
            for (var i = 0; i < this.records.length; i++) {
                if (this.records[i].officename == officename) {
                    if (returnIndex === true) return i; else return this.records[i];
                }
            }
            // search summary
            for (var i = 0; i < this.summary.length; i++) {
                if (this.summary[i].officename == officename) {
        ...
        remove: function () {
            ...
            for (var a = 0; a < arguments.length; a++) {
                for (var r = this.records.length-1; r >= 0; r--) {
                    if (this.records[r].officename == arguments[a]) { this.records.splice(r, 1); removed++; }
                }
            }
            var url = (typeof this.url != 'object' ? this.url : this.url.get);
            if (!url) {
        ...
        getRangeData: function (range, extra) {
            var rec1 = this.get(range[0].officename, true);
            var rec2 = this.get(range[1].officename, true);
            ...
            for (var r in ranges) {
                ...
                if (ranges[r].name == 'selection') {
                    ...
                    if (sel.length == 0) {
                        ...
                        var td1   = $('#grid_'+ this.name +'_rec_'+ first.officename + ' td[col='+ first.column +']');
                        var td2   = $('#grid_'+ this.name +'_rec_'+ last.officename + ' td[col='+ last.column +']');
                    }
                } else { // other range
                ...
                if (first) {
                    var rg = {
                        name: ranges[r].name,
                        range: [{ officename: first.officename, column: first.column }, { officename: last.officename, column: last.column }],
        ...
        refreshRanges: function () {
            ...
            function mouseStart (event) {
                ...
                obj.last.move = {
                    ...
                    divX   : 0,
                    divY   : 0,
                    officename  : sel[0].officename,
                    column : sel[0].column,
                    originalRange : [{ officename: sel[0].officename, column: sel[0].column }, { officename: sel[sel.length-1].officename, column: sel[sel.length-1].column }],
                    newRange      : [{ officename: sel[0].officename, column: sel[0].column }, { officename: sel[sel.length-1].officename, column: sel[sel.length-1].column }]
                };
            ...
            function mouseMove (event) {
                ...
                officename = $(tmp).attr('officename');
                // new range
                if (mv.newRange[1].officename == officename && mv.newRange[1].column == column) return;
                var prevNewRange = $.extend({}, mv.newRange);
                mv.newRange = [{ officename: mv.officename, column: mv.column }, { officename: officename, column: column }];
                // event before
        ...
        select: function () {
            ...
            for (var a = 0; a < arguments.length; a++) {
                var officename  = typeof arguments[a] == 'object' ? arguments[a].officename : arguments[a];
                var record = this.get(officename);
                if (record == null) continue;
                var index  = this.get(officename, true);
                var recEl  = $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(officename));
                if (this.selectType == 'row') {
                    if (sel.indexes.indexOf(index) >= 0) continue;
                    // event before
                    var eventData = this.trigger({ phase: 'before', type: 'select', target: this.name, officename: officename, index: index });
                    ...
                    if (!w2utils.isInt(col)) { // select all columns
                        var cols = [];
                        for (var c in this.columns) { if (this.columns[c].hidden) continue; cols.push({ officename: officename, column: parseInt(c) }); }
                    ...
                    // event before
                    var eventData = this.trigger({ phase: 'before', type: 'select', target: this.name, officename: officename, index: index, column: col });
                    if (eventData.isCancelled === true) continue;
                    // default action
        ...
        unselect: function () {
            ...
            for (var a = 0; a < arguments.length; a++) {
                ...
                var record = this.get(officename);
                if (record == null) continue;
                var index  = this.get(record.officename, true);
                var recEl  = $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(officename));
                if (this.selectType == 'row') {
                    if (sel.indexes.indexOf(index) == -1) continue;
                    // event before
                    var eventData = this.trigger({ phase: 'before', type: 'unselect', target: this.name, officename: officename, index: index });
                    if (eventData.isCancelled === true) continue;
                    // default action
                    ...
                    if (!$.isArray(s) || s.indexOf(col) == -1) continue;
                    // event before
                    var eventData = this.trigger({ phase: 'before', type: 'unselect', target: this.name, officename: officename, column: col });
                    if (eventData.isCancelled === true) continue;
                    // default action
                    s.splice(s.indexOf(col), 1);
                    $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(officename) + ' > td[col='+ col +']').removeClass('w2ui-selected');
                    unselected++;
                    if (s.length == 0) {
        ...
        selectNone: function () {
            ...
            for (var s in sel.indexes) {
                ...
                var officename = rec ? rec.officename : null;
                var recEl = $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(officename));
                recEl.removeClass('w2ui-selected').removeData('selected');
                recEl.find('.w2ui-grid-select-check').prop("checked", false);
                // for not rows
        ...
        getSelection: function (returnIndex) {
            ...
            if (this.selectType == 'row') {
                for (var s in sel.indexes) {
                    ...
                    if (returnIndex === true) ret.push(sel.indexes[s]); else ret.push(this.records[sel.indexes[s]].officename);
                }
                return ret;
            } else {
                for (var s in sel.indexes) {
                    ...
                    if (!this.records[sel.indexes[s]]) continue;
                    for (var c in cols) {
                        ret.push({ officename: this.records[sel.indexes[s]].officename, index: parseInt(sel.indexes[s]), column: cols[c] });
                    }
                }
                return ret;
            }
                ...
                .fail(function (xhr, status, error) {
                    ...
                    if (status != 'abort') {
                        ...
                        console.log('ERROR: Server communication failed.', 
                            '\n   EXPECTED:', { status: 'success', total: 5, records: [{ officename: 1, field: 'value' }] }, 
                ...
                if (typeof responseText != 'undefined' && responseText != '') {
                    ...
                    if (obj.officename) {
                        for (var r in data.records) {
                            data.records[r]['officename'] = data.records[r][obj.officename];
                        }
                    }
        ...
        getChanges: function () {
            ...
            for (var r in this.records) {
                ...
                if (typeof rec['changes'] != 'undefined') {
                    changes.push($.extend(true, { officename: rec.officename }, rec.changes));
                }
            }
            return changes;
        },
        ...
        mergeChanges: function () {
            ...
            for (var c in changes) {
                var record = this.get(changes[c].officename);
                for (var s in changes[c]) {
                    if (s == 'officename') continue; // do not allow to change officename
        ...
        editField: function (officename, column, value, event) {
            var obj   = this;
            var index = obj.get(officename, true);
            ...
            // event before
            var eventData = obj.trigger({ phase: 'before', type: 'editField', target: obj.name, officename: officename, column: column, value: value,
                index: index, originalEvent: event });
            if (eventData.isCancelled === true) return;
            value = eventData.value;
            // default behaviour
            this.selectNone();
            this.select({ officename: officename, column: column });
            this.last.edit_col = column;
            if (['checkbox', 'check'].indexOf(edit.type) != -1) return;
            // create input element
            var tr = $('#grid_'+ obj.name +'_rec_'+ w2utils.escapeId(officename));
            ...
            if (edit.type == 'select') {
                ...
                el.addClass('w2ui-editable')
                    .html('<select id="grid_'+ obj.name +'_edit_'+ officename +'_'+ column +'" column="'+ column +'" '+
                        '    style="width: 100%; '+ addStyle + edit.style +'" field="'+ col.field +'" officename="'+ officename +'" '+
                        '    '+ edit.inTag +
                ...
                el.addClass('w2ui-editable')
                    .html('<input id="grid_'+ obj.name +'_edit_'+ officename +'_'+ column +'" '+
                        '    type="text" style="font-family: inherit; font-size: inherit; outline: none; '+ addStyle + edit.style +'" field="'+ col.field +'" officename="'+ officename +'" '+
                    ...
                    .on('keydown', function (event) {
                        ...
                        switch (event.keyCode) {
                            case 9:  // tab
                                ...
                                if (next_col == null) {
                                    ...
                                    if (tmp != null && tmp != index) {
                                        next_rec = obj.records[tmp].officename;
                                ...
                                if (next_rec === false) next_rec = officename;
                                ...
                                setTimeout(function () {
                                    if (obj.selectType != 'row') {
                                        obj.selectNone();
                                        obj.select({ officename: next_rec, column: next_col });
                                    } else {
                            ...
                            case 13: // enter
                                ...
                                if (next != null && next != index) {
                                    setTimeout(function () {
                                        if (obj.selectType != 'row') {
                                            obj.selectNone();
                                            obj.select({ officename: obj.records[next].officename, column: column });
                                        } else {
                                            obj.editField(obj.records[next].officename, column, null, event);
                                        }
                                    }, 100);
                                }
                                break;
                            ...
                            case 27: // escape
                                ...
                                this.blur();
                                setTimeout(function () { obj.select({ officename: officename, column: column }) }, 1);
                                break;
                        }
        ...
        editChange: function (el, index, column, event) {
            ...
            var tr      = $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(rec.officename));
            ...
            var eventData = {
                phase: 'before', type: 'change', target: this.name, input_id: el.id, officename: rec.officename, index: index, column: column,
            ...
            if (url) {
                ...
                if (typeof recs[0] != 'object') {
                    ...
                    for (var r in recs) {
                        ...
                        var ind = this.get(recs[r].officename, true);
                        if (ind != null && fld != 'officename') {
        ...
        click: function (officename, event) {
            ...
            // event before
            var eventData = this.trigger({ phase: 'before', target: this.name, type: 'click', officename: officename, column: column, originalEvent: event });
            if (eventData.isCancelled === true) return;
            // if it is subgrid unselect top grid
            var parent = $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(officename)).parents('tr');
            ...
            obj.last.sel_officename = officename;
            obj.last.sel_type  = 'click';
            // multi select with shif key
            if (event.shiftKey && sel.length > 0 && obj.multiSelect) {
                if (sel[0].officename) {
                    var start = this.get(sel[0].officename, true);
                    var end   = this.get(officename, true);
                ...
                for (var i = start; i <= end; i++) {
                    ...
                    if (this.selectType == 'row') {
                        sel_add.push(this.records[i].officename);
                    } else {
                        for (var sc in selectColumns) sel_add.push({ officename: this.records[i].officename, column: selectColumns[sc] });
                    }
                    //sel.push(this.records[i].officename);
                }
                ...
                if (((!event.ctrlKey && !event.shiftKey && !event.metaKey) || !this.multiSelect) && !this.showSelectColumn) {
                    ...
                    if (flag === true) {
                        this.unselect({ officename: officename, column: column });
                    } else {
                        this.select({ officename: officename, column: column });
                    }
                } else {
        ...
        keydown: function (event) {
            ...
            var officename   = sel[0] || null;
            var columns = [];
            var officename2  = sel[sel.length-1];
            if (typeof officename == 'object' && officename != null) {
                officename   = sel[0].officename;
                columns = [];
                var ii  = 0;
                while (true) {
                    if (!sel[ii] || sel[ii].officename != officename) break;
                    columns.push(sel[ii].column);
                    ii++;
                }
                officename2  = sel[sel.length-1].officename;
            }
            var ind    = obj.get(officename, true);
            var ind2   = obj.get(officename2, true);
            var rec    = obj.get(officename);
            var recEL  = $('#grid_'+ obj.name +'_rec_'+ (ind !== null ? w2utils.escapeId(obj.records[ind].officename) : 'none'));
            var cancel = false;
            var key    = event.keyCode;
            ...
            switch (key) {
                ...
                case 27: // escape
                    ...
                    if (sel.length > 0 && typeof sel[0] == 'object') {
                        obj.select({ officename: sel[0].officename, column: sel[0].column });
                    }
                    cancel = true;
                    break;
                ...
                case 13: // enter
                    ...
                    if (this.selectType == 'row' && obj.show.expandColumn === true) {
                        ...
                        if (columns.length > 0) {
                            obj.editField(officename, columns[0], null, event);
                            cancel = true;
                        }
                    }
                    break;
                ...
                case 37: // left
                    ...
                    var parent = $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(obj.records[ind].officename)).parents('tr');
                    if (parent.length > 0 && String(parent.attr('id')).indexOf('expanded_row') != -1) {
                        var officename = parent.prev().attr('officename');
                        var grid  = parent.parents('.w2ui-grid').attr('name');
                        obj.selectNone();
                        w2utils.keyboard.active(grid);
                        w2ui[grid].set(officename, { expanded: false });
                        w2ui[grid].collapse(officename);
                        w2ui[grid].click(officename);
                        cancel = true;
                        break;
                    }
                    if (this.selectType == 'row') {
                        if (recEL.length <= 0 || rec.expanded !== true ) break;
                        obj.set(officename, { expanded: false }, true);
                        obj.collapse(officename, event);
                    } else {
                        ...
                        if (prev != null) {
                            if (shiftKey && obj.multiSelect) {
                                ...
                                if (columns.indexOf(this.last.sel_col) == 0 && columns.length > 1) {
                                    for (var i in sel) {
                                        if (tmp.indexOf(sel[i].officename) == -1) tmp.push(sel[i].officename);
                                        unSel.push({ officename: sel[i].officename, column: columns[columns.length-1] });
                                    }
                                } else {
                                    for (var i in sel) {
                                        if (tmp.indexOf(sel[i].officename) == -1) tmp.push(sel[i].officename);
                                        newSel.push({ officename: sel[i].officename, column: prev });
                                    }
                                }
                                ...
                                obj.click({ officename: officename, column: prev }, event);
                            }
                        } else {
                        ...
                        if (next !== null) {
                            if (shiftKey && key == 39 && obj.multiSelect) {
                                ...
                                if (columns.indexOf(this.last.sel_col) == columns.length-1 && columns.length > 1) {
                                    for (var i in sel) {
                                        if (tmp.indexOf(sel[i].officename) == -1) tmp.push(sel[i].officename);
                                        unSel.push({ officename: sel[i].officename, column: columns[0] });
                                    }
                                } else {
                                    for (var i in sel) {
                                        if (tmp.indexOf(sel[i].officename) == -1) tmp.push(sel[i].officename);
                                        newSel.push({ officename: sel[i].officename, column: next });
                                    }
                                }
                                obj.unselect.apply(obj, unSel);
                                obj.select.apply(obj, newSel);
                            } else {
                                obj.click({ officename: officename, column: next }, event);
                            }
                        } else {
                            // if selected more then one, then select first
                            if (!shiftKey) {
                ...
                case 38: // up
                    ...
                    if (prev != null) {
                        ...
                        if (obj.records[prev].expanded) {
                            var subgrid = $('#grid_'+ obj.name +'_rec_'+ w2utils.escapeId(obj.records[prev].officename) +'_expanded_row').find('.w2ui-grid');
                            if (subgrid.length > 0 && w2ui[subgrid.attr('name')]) {
                                obj.selectNone();
                                var grid = subgrid.attr('name');
                                var recs = w2ui[grid].records;
                                w2utils.keyboard.active(grid);
                                w2ui[grid].click(recs[recs.length-1].officename);
                                cancel = true;
                                break;
                            }
                        }
                        if (shiftKey && obj.multiSelect) { // expand selection
                            ...
                            if (obj.selectType == 'row') {
                                if (obj.last.sel_ind > prev && obj.last.sel_ind != ind2) {
                                    obj.unselect(obj.records[ind2].officename);
                                } else {
                                    obj.select(obj.records[prev].officename);
                                }
                            } else {
                                if (obj.last.sel_ind > prev && obj.last.sel_ind != ind2) {
                                    prev = ind2;
                                    var tmp = [];
                                    for (var c in columns) tmp.push({ officename: obj.records[prev].officename, column: columns[c] });
                            ...
                            obj.selectNone();
                            obj.click({ officename: obj.records[prev].officename, column: columns[0] }, event);
                        }
                        ...
                        var parent = $('#grid_'+ obj.name +'_rec_'+ w2utils.escapeId(obj.records[ind].officename)).parents('tr');
                        if (parent.length > 0 && String(parent.attr('id')).indexOf('expanded_row') != -1) {
                            var officename = parent.prev().attr('officename');
                            var grid  = parent.parents('.w2ui-grid').attr('name');
                            obj.selectNone();
                ...
                case 40: // down
                    ...
                    if (obj.records[ind2].expanded) {
                        var subgrid = $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(obj.records[ind2].officename) +'_expanded_row').find('.w2ui-grid');
                        if (subgrid.length > 0 && w2ui[subgrid.attr('name')]) {
                            obj.selectNone();
                            var grid = subgrid.attr('name');
                            var recs = w2ui[grid].records;
                            w2utils.keyboard.active(grid);
                            w2ui[grid].click(recs[0].officename);
                            cancel = true;
                            break;
                        }
                    }
                    ...
                    if (next != null) {
                        if (shiftKey && obj.multiSelect) { // expand selection
                            ...
                            if (obj.selectType == 'row') {
                                if (this.last.sel_ind < next && this.last.sel_ind != ind) {
                                    obj.unselect(obj.records[ind].officename);
                                } else {
                                    obj.select(obj.records[next].officename);
                                }
                            } else {
                                if (this.last.sel_ind < next && this.last.sel_ind != ind) {
                                    next = ind;
                                    var tmp = [];
                                    for (var c in columns) tmp.push({ officename: obj.records[next].officename, column: columns[c] });
                                    obj.unselect.apply(obj, tmp);
                                } else {
                                    var tmp = [];
                                    for (var c in columns) tmp.push({ officename: obj.records[next].officename, column: columns[c] });
                            ...
                            obj.selectNone();
                            obj.click({ officename: obj.records[next].officename, column: columns[0] }, event);
                        }
                        ...
                        var parent = $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(obj.records[ind2].officename)).parents('tr');
                        if (parent.length > 0 && String(parent.attr('id')).indexOf('expanded_row') != -1) {
                            var officename = parent.next().attr('officename');
                            var grid  = parent.parents('.w2ui-grid').attr('name');
                            obj.selectNone();
                            w2utils.keyboard.active(grid);
                            w2ui[grid].click(officename);
            ...
            if (tmp.indexOf(key) != -1 && !event.ctrlKey && !event.metaKey && !cancel) {
                ...
                obj.editField(officename, columns[0], tmp, event);
                cancel = true;
            }
            ...
            function selectTopRecord() {
                ...
                obj.select({ officename: obj.records[ind].officename, column: 0});
            }
            ...
            function tmpUnselect () {
                ...
                if (obj.selectType != 'row') {
                    ...
                    if (sel.length > 1) {
                        for (var s in sel) {
                            if (sel[s].officename == obj.last.sel_officename && sel[s].column == obj.last.sel_col) {
                                sel.splice(s, 1);
                                break;
                            }
                        }
                    ...
                    obj.last.sel_type = 'key';
                    if (sel.length > 1) {
                        sel.splice(sel.indexOf(obj.records[obj.last.sel_ind].officename), 1);
                        obj.unselect.apply(obj, sel);
                        return true;
                    }
                    return false;
        ...
        dblClick: function (officename, event) {
            ...
            // event before
            var eventData = this.trigger({ phase: 'before', target: this.name, type: 'dblClick', officename: officename, column: column, originalEvent: event });
            if (eventData.isCancelled === true) return;
            // default action
            this.selectNone();
            var col = this.columns[column];
            if (col && $.isPlainObject(col.editable)) {
                this.editField(officename, column, null, event);
            } else {
                this.select({ officename: officename, column: column });
            }
            // event after
        ...
        contextMenu: function (officename, event) {
            var obj = this;
            if (obj.last.userSelect == 'text') return;
            if (typeof event == 'undefined') event = { offsetX: 0, offsetY: 0, target: $('#grid_'+ obj.name +'_rec_'+ officename)[0] };
            if (typeof event.offsetX === 'undefined') {
                event.offsetX = event.layerX - event.target.offsetLeft;
                event.offsetY = event.layerY - event.target.offsetTop;
            }
            if (w2utils.isFloat(officename)) officename = parseFloat(officename);
            if (this.getSelection().indexOf(officename) == -1) obj.click(officename);
            // need timeout to allow click to finish first
            setTimeout(function () {
                // event before
                var eventData = obj.trigger({ phase: 'before', type: 'contextMenu', target: obj.name, originalEvent: event, officename: officename });
                if (eventData.isCancelled === true) return;
                // default action
                    ...
                    $(obj.box).find(event.target)
                        .w2menu(obj.menu, {
                            ...
                            onSelect: function (event) { 
                                obj.menuClick(officename, parseInt(event.index), event.originalEvent); 
                            }
                        }
                    );
                }
        ...
        menuClick: function (officename, index, event) {
            var obj = this;
            // event before
            var eventData = obj.trigger({ phase: 'before', type: 'menuClick', target: obj.name, originalEvent: event, 
                officename: officename, menuIndex: index, menuItem: obj.menu[index] });
            if (eventData.isCancelled === true) return;
            // default action
            // -- empty
            // event after
        ...
        toggle: function (officename) {
            var rec = this.get(officename);
            if (rec.expanded === true) return this.collapse(officename); else return this.expand(officename);
        },

        expand: function (officename) {
            var rec = this.get(officename);
            var obj = this;
            var id  = w2utils.escapeId(officename);
            ...
            var addClass = ''; // ($('#grid_'+this.name +'_rec_'+ w2utils.escapeId(officename)).hasClass('w2ui-odd') ? 'w2ui-odd' : 'w2ui-even');
            $('#grid_'+ this.name +'_rec_'+ id).after(
                    '<tr id="grid_'+ this.name +'_rec_'+ officename +'_expanded_row" class="w2ui-expanded-row '+ addClass +'">'+
                        (this.show.lineNumbers ? '<td class="w2ui-col-number"></td>' : '') +
                    '    <td class="w2ui-grid-data w2ui-expanded1" colspan="'+ tmp +'"><div style="display: none"></div></td>'+
                    '    <td colspan="100" class="w2ui-expanded2">'+
                    '        <div id="grid_'+ this.name +'_rec_'+ officename +'_expanded" style="opacity: 0"></div>'+
                    '    </td>'+
                    '</tr>');
            // event before
            var eventData = this.trigger({ phase: 'before', type: 'expand', target: this.name, officename: officename,
                box_id: 'grid_'+ this.name +'_rec_'+ officename +'_expanded', ready: ready });
            if (eventData.isCancelled === true) {
            ...
            $('#grid_'+ this.name +'_cell_'+ this.get(officename, true) +'_expand div').html('<div class="w2ui-spinner" style="width: 16px; height: 16px; margin: -2px 2px;"></div>');
            rec.expanded = true;
            ...
            function ready() {
                ...
                $('#grid_'+ obj.name +'_cell_'+ obj.get(officename, true) +'_expand div').html('-');
            }
            // event after
            this.trigger($.extend(eventData, { phase: 'after' }));
            this.resizeRecords();
        ...
        collapse: function (officename) {
            var rec = this.get(officename);
            var obj = this;
            var id  = w2utils.escapeId(officename);
            if ($('#grid_'+ this.name +'_rec_'+ id +'_expanded_row').length == 0) return false;
            // event before
            var eventData = this.trigger({ phase: 'before', type: 'collapse', target: this.name, officename: officename,
            ...
            $('#grid_'+ this.name +'_cell_'+ this.get(officename, true) +'_expand div').html('+');
            setTimeout(function () {
        ...
        paste: function (text) {
            var sel = this.getSelection();
            var ind = this.get(sel[0].officename, true);
            ...
            for (var t in text) {
                ...
                for (var c in cols) newSel.push({ officename: rec.officename, column: cols[c] });
                ind++;
            }
            this.selectNone();
        ...
        refreshCell: function (officename, field) {
            var index     = this.get(officename, true);
            var isSummary = (this.records[index] && this.records[index].officename == officename ? false : true);
            var col_ind   = this.getColumn(field, true);
            var rec       = (isSummary ? this.summary[index] : this.records[index]);
            var col       = this.columns[col_ind];
            var cell      = $('#grid_'+ this.name + '_rec_'+ officename +' [col='+ col_ind +']');
        ...
        refreshRow: function (officename) {
            var tr = $('#grid_'+ this.name +'_rec_'+ w2utils.escapeId(officename));
            if (tr.length != 0) {
                var ind  = this.get(officename, true);
        ...
        render: function (box) {
            ...
            function mouseStart (event) {
                ...
                if (event.altKey) {
                    ...
                    obj.last.move = {
                        ...
                        divX   : 0,
                        divY   : 0,
                        officename  : $(event.target).parents('tr').attr('officename'),
            ...
            function mouseMove (event) {
                ...
                if (obj.reorderRows == true) {
                    ...
                    var officename = $(event.target).parents('tr').attr('officename');
                    if (officename != mv.from) {
                        var row1 = $('#grid_'+ obj.name + '_rec_'+ mv.officename);
                        var row2 = $('#grid_'+ obj.name + '_rec_'+ officename);
                ...
                if (mv.start && mv.officename) {
                    obj.selectNone();
                    mv.start = false;
                }
                var newSel= [];
                var officename = (event.target.tagName == 'TR' ? $(event.target).attr('officename') : $(event.target).parents('tr').attr('officename'));
                if (typeof officename == 'undefined') return;
                var ind1  = obj.get(mv.officename, true);
                // |:wolfmanx:| this happens when selection is started on summary row
                if (ind1 === null) return;
                var ind2  = obj.get(officename, true);
                ...
                for (var i = ind1; i <= ind2; i++) {
                    ...
                    if (obj.selectType != 'row') {
                        ...
                        for (var c = col1; c <= col2; c++) {
                            ...
                            newSel.push({ officename: obj.records[i].officename, column: parseInt(c) });
                        }
                    } else {
                        newSel.push(obj.records[i].officename);
                    }
                }
                if (obj.selectType != 'row') {
                    var sel = obj.getSelection();
                    ...
                    for (var ns in newSel) {
                        var flag = false;
                        for (var s in sel) if (newSel[ns].officename == sel[s].officename && newSel[ns].column == sel[s].column) flag = true;
                        if (!flag) tmp.push({ officename: newSel[ns].officename, column: newSel[ns].column });
                    }
                    obj.select.apply(obj, tmp);
                    // remove items
                    var tmp = [];
                    for (var s in sel) {
                        var flag = false;
                        for (var ns in newSel) if (newSel[ns].officename == sel[s].officename && newSel[ns].column == sel[s].column) flag = true;
                        if (!flag) tmp.push({ officename: sel[s].officename, column: sel[s].column });
                    }
                    obj.unselect.apply(obj, tmp);
                } else {
                    if (obj.multiSelect) {
            ...
            if (typeof this.toolbar['render'] == 'undefined') {
                ...
                this.toolbar.on('click', function (event) {
                    ...
                    switch (id) {
                        ...
                        case 'w2ui-add':
                            // events
                            var eventData = obj.trigger({ phase: 'before', target: obj.name, type: 'add', officename: null });
                            obj.trigger($.extend(eventData, { phase: 'after' }));
                            break;
                        case 'w2ui-edit':
                            var sel     = obj.getSelection();
                            var officename     = null;
                            if (sel.length == 1) officename = sel[0];
                            // events
                            var eventData = obj.trigger({ phase: 'before', target: obj.name, type: 'edit', officename: officename });
                            obj.trigger($.extend(eventData, { phase: 'after' }));
                            break;
                        case 'w2ui-delete':
                            obj["delete"]();
        ...
        getRecordHTML: function (ind, lineNum, summary) {
            ...
            if (!record) return '';
            var id = w2utils.escapeId(record.officename);
            var isRowSelected = false;
            if (sel.indexes.indexOf(ind) != -1) isRowSelected = true;
            // render TR
            rec_html += '<tr id="grid_'+ this.name +'_rec_'+ record.officename +'" officename="'+ record.officename +'" line="'+ lineNum +'" '+
                ' class="'+ (lineNum % 2 == 0 ? 'w2ui-even' : 'w2ui-odd') + (isRowSelected && this.selectType == 'row' ? ' w2ui-selected' : '') + (record.expanded === true ? ' w2ui-expanded' : '') + '" ' +
                (summary !== true ?
                    (w2utils.isIOS ?
                        '    onclick  = "w2ui[\''+ this.name +'\'].dblClick(\''+ record.officename +'\', event);"'
                        :
                        '    onclick  = "w2ui[\''+ this.name +'\'].click(\''+ record.officename +'\', event);"'+
                        '    oncontextmenu = "w2ui[\''+ this.name +'\'].contextMenu(\''+ record.officename +'\', event);"'
                     )
                    : ''
                ) +
            ...
            if (this.show.selectColumn) {
                rec_html +=
                        ...
                        '        onclick="if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;">'+
                            ...
                            '                if (this.checked) obj.select(\''+ record.officename + '\'); else obj.unselect(\''+ record.officename + '\'); '+
                            '                if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;">'+
                            '    </div>'
                            :
                            '' ) +
            ...
            if (this.show.expandColumn) {
                ...
                rec_html +=
                        '<td id="grid_'+ this.name +'_cell_'+ ind +'_expand' + (summary ? '_s' : '') + '" class="w2ui-grid-data w2ui-col-expand">'+
                            ...
                            '            onclick="w2ui[\''+ this.name +'\'].toggle(\''+ record.officename +'\', event); '+
                            '                if (event.stopPropagation) event.stopPropagation(); else event.cancelBubble = true;">'+
                            '        '+ tmp_img +' </div>'
                            :
                            '' ) +
        ...
        status: function (msg) {
            if (typeof msg != 'undefined') {
                ...
                if (sel.length > 0) {
                    ...
                    var tmp = sel[0];
                    if (typeof tmp == 'object') tmp = tmp.officename + ', '+ w2utils.lang('Column') +': '+ tmp.column;
...
(function () {
    var w2form = function(options) {
        ...
        this.officename     = 0;        // can be null or 0
        this.fields    = [];
        this.actions   = {};
        this.record    = {};
        this.original  = {};
    ...
    w2form.prototype = {
        ...
        request: function (postData, callBack) { // if (1) param then it is call back if (2) then postData and callBack
            ...
            if (this.officename == null || typeof this.officename == 'undefined') this.officename = 0;
            // build parameters list
            var params = {};
            // add list params
        ...
        save: function (postData, callBack) {
            ...
            setTimeout(function () {
                ...
                params['officename'] = obj.officename;
                // append other params
                ...
                if (w2utils.settings.dataType == 'RESTFULL') {
                    if (obj.officename != 0) ajaxOptions.type = 'PUT';
                    ajaxOptions.data = String($.param(ajaxOptions.data, false)).replace(/%5B/g, '[').replace(/%5D/g, ']');
                }
                if (w2utils.settings.dataType == 'JSON') {
                    ajaxOptions.type        = 'POST';
